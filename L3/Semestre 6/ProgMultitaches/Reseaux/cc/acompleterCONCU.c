...
int sendTCP(int sock, void *msg, int sizeMsg){
  ...
  else {return sendTCP(sock,(void *)((int *)msg+w),sizeMsg-w);} //Dépôt incomplet
}
  ...
  */
  ...
  if(recvfromsize==-1){
   ...
   std::cout << "\nL'expéditeur est :\n" <<  inet_ntoa(sockaddrRetour.sin_addr) << "\n" <<  ntohs(sockaddrRetour.sin_port) << "\n";
    ...
    //On créer la socket de l'expéditeur
  }
  ...
  while(1){
    ...
    dSC = accept(dsTCP,(struct sockaddr *)&sockaddrRetour,&lgRetour); /*On accepte la connexion*/
    if(dSC==-1){
  ...
  if(f==0){
    ...
    if(sendsize<=0){
      ...
      std::cout << "Le tableau d'entiers a bien été envoyé.\n";
    }
    ...
    if(recvsize2<=0){
      ...
      std::cout << "Un message de " << tailleRep2 << " octets va être envoyé.\n";
    }
