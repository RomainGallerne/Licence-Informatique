...
int main(int argc, char *argv[]) {
  ...
  if (argc != 5){
    printf("utilisation : %s ip_serveur port_serveur port_client nb_envoi\n", argv[0]);
    exit(1);
  }
  ...
  int nbSend=0;
  int nb_octet_total=0;
  msg[strlen(msg)+1] = '\0';

  std::cout << "Taille d'un unique message = " << strlen(msg)+1 << "\n";
  std::cout << "Nombre d'octets supposés être envoyés = " << atoi(argv[4])*(strlen(msg)+1) << "\n";
  ...
  for(int i=0;i<atoi(argv[4]);i++){
    ...
    if(sendsize<=0){
      ...
      nbSend += 1;
      nb_octet_total += sendsize;
    }

  }
  std::cout << nbSend << " appels de send ont été réalisés.\n";
  std::cout << nb_octet_total << " octets ont été envoyés au total.\n";
