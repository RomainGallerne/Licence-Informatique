...
int main(int argc, char *argv[]) {
  ...
  int tailleRep;
  int recvsize = recv(dSC, &tailleRep, sizeof(int),0);
  if(recvsize<=0){
    perror("Serveur : Erreur du retour de la taille du message\n");
    close(ds);
    close(dSC);
    exit(1);
  } else {
    std::cout << "Un message de " << tailleRep << " octets va être envoyé.\n";
  }

  /*Recpetion du message*/
  char rep [tailleRep];
  recvsize = recv(dSC, &rep, tailleRep,0);
  if(recvsize<=0){
  ...
  //std::cin >> msg;
  std::cout << recvsize << "\n";
  int sendsize = send(dSC,&recvsize,sizeof(int),0); /*strlen(msg)+1*/
  if(sendsize<=0){
