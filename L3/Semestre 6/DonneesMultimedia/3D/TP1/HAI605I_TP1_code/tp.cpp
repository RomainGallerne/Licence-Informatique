...
{   
    ...
    for (float p=-pi/2.f; p<=pi/2.f; p+=(pi/NY)){
        for (float t=0.f; t<=2.f*pi; t+=(2.f*pi/NX)){
            Px = cos(t) * cos(p);
            Py = sin(t) * cos(p);
            Pz = sin(p);
            o_mesh.vertices.push_back(Vec3(Px,Py,Pz));
            o_mesh.normals.push_back(Vec3(Px,Py,Pz));
        }
    }
    for (int y=0; y<NY; y+=1){ //Pour chaque parrallèle
        for (int x=0; x<NX; x+=1){ //Pour chaque méridien
            p1 = x+(y*NY);

            if(p1 == ((y+1)*NY)-1){ //Cas bord de méridien
                p2 = p1-NX+1;
                p3 = p1+NX;
                p4 = p1+1;
            }
            ...
            if(p3<=NX*(NY+1)){o_mesh.triangles.push_back(Triangle(p1, p2, p3));}
            if(p4<=NX*(NY+1)){o_mesh.triangles.push_back(Triangle(p2, p4, p3));}
        }
    }
}
...
void key (unsigned char keyPressed, int x, int y) {
    ...
    case '+': //Augmenter le nombre de points
        nX += 1;
        nY += 1;
        setUnitSphere(unit_sphere);
        break;

    case '-': //Diminuer le nombre de points
        nX -= 1;
        nY -= 1;
        setUnitSphere(unit_sphere);
        break;
